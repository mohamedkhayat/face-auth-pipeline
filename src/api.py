...
backbone = resnet50()
model = FaceVerificationModel(backbone, dropout=0.3, embedding_size=config.EMB_DIM).to(device)

mtcnn = MTCNN(keep_all=False, device=device)

model.load_state_dict(torch.load("./models/face_verification_resnet50.pth", map_location=device))
...
async def compare_faces(ref_embedding: str = Form(...), 
    ...
    try:
        ...
        raise HTTPException(status_code=400,detail=f"Error reading embedding : {str(e)}")
