...
def margin_schedule(epoch, initial_margin=config.INITIAL_MARGIN, peak_margin=config.PEAK_MARGIN, final_margin=config.FINAL_MARGIN, peak_epoch=15, total_epochs=config.N_EPOCHS_MARGIN):
    if epoch < peak_epoch:
        return initial_margin + (peak_margin - initial_margin) * (epoch / peak_epoch)  # Gradual increase
    else:
        return peak_margin - ((peak_margin - final_margin) * ((epoch - peak_epoch) / (total_epochs - peak_epoch)))
      

class EarlyStopping:
    def __init__(self, patience=5, min_delta=0.0, path='./models/earlystopping_model.pth'):
...
def plot_and_log_tsne(embeddings, labels, name, perplexity=30, artifact_name="tsne_plots"):
...
def evaluate_model(model, device, dataset_path, test_transforms, name):
    ...
    plot_and_log_tsne(embeddings, labels, name=name, perplexity=40, artifact_name="tsne_" + name)

    similarities, true_labels = compute_cosine_similarities(label_to_imgs, model, test_transforms, device)

    best_thresh, best_acc, thresholds, acc_list = find_best_threshold(similarities, true_labels)
